...
const Months = () => {
  ...
  const [availableotdata,setavailableotdata] = useState([]);
  const [BookedDates,setBookedDates] = useState([]);

  console.log(Theatredata,'handleTheatreChange');
   
  console.log(availableotdata,'availableotdata');
  ...
  useEffect(() => {
    axios
      .get(`${UrlLink}Masters/Surgeon_list_for_calendar?desname=${Selectedcrewname}`)
  ...
  useEffect(() => {
    axios
      .get(`${UrlLink}Masters/get_ot_doctor_availability?docid=${SelectedDoctor}`)
      .then((res) => {
        console.log(res.data);
        const data = res.data;
        setavailableotdata(res.data);
        const dates = data.map((entry) => entry.bdate); 
        console.log(dates,'premkumar');        // Extract the 'bdate' from API response
        setBookedDates(dates); // Set the booked dates in state
      })
      
      .catch((err) => console.log(err));
  }, [UrlLink,SelectedDoctor]);
  ...
  useEffect(() => {
    const daysInMonth = () => {
      ...
      const startingDay = new Date(year, month, 1).getDay(); // Ensure this is correct
      const daysArray = [];
      ...
      // Add empty cells for the first row until the starting day
      for (let i = 0; i < startingDay; i++) {
        daysArray.push(null);
      }
  
      // Add day objects for the rest of the days in the month
      for (let day = 1; day <= lastDay; day++) {
        const date = new Date(year, month, day +1).toISOString().split("T")[0];
       
        
  
        // Find the corresponding entry in availableotdata
        const bookedEntry = availableotdata.find((entry) => entry.bdate === date);
  
        daysArray.push({
          date: date,
          time : bookedEntry ? bookedEntry.btime  : null,
          patientname : bookedEntry ? bookedEntry.patientname  : null,
          status: bookedEntry ? "Booked" : "Available",
          priority: bookedEntry ? bookedEntry.priority : null,
          operationType: bookedEntry ? bookedEntry.operationtype : null,
        });
      }
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        const app =
          Array.isArray(ress) &&
          ress.filter((apps) => apps.DoctorID === DoctorListId.DoctorId);
  ...
  }, [UrlLink, DoctorListId.DoctorId, GetDoctorData]);
      ...
      const fetchUpdatedDoctorDetails = async () => {
        try {
          ...
          const updatedDoctorappdetails = processedDoctors.filter(
            (doc) => doc.doctor_id === DoctorListId.DoctorId
          );
  ...
  //       const isToday = dayDate.getTime() === today.getTime(); // Check if the date is today
  //       const isBeforeToday = dayDate.getTime() < today.getTime(); // Check if the date is before today
  ...
  //             </button>
  //             <div className="calender_table_locations">
  //               <label htmlFor="names">
  //                 Select Theatre <span>:</span>
  //               </label>
  //               <select
  //                 className="calender_table_locations_select"
  //                 name="Doctor Location"
  //                 onChange={(e) => handleTheatreChange(e)}
  //                 value={SelectedTheater}
  //               >
  //                 <option value="">Select</option>
  //                 {Array.isArray(Theatredata) && (Theatredata).map((p, indx) => (
  ...
  const renderCalendar = () => {
    ...
    calendarDays.forEach((day, index) => {
      ...
      if (day) {
        ...
        const isBeforeToday = dayDate < new Date();
        const changedClassName = day.Changed ? "changed_text" : "";
  
        cells.push(
          <td
            key={index}
            className={`cal_flex ${day.status === "Booked" ? "booked" : "available"}
            ${isToday ? "today_date" : ""}
            ${isBeforeToday ? "previous_date" : ""}`}
            onContextMenu={(e) => handleCellClick(e, day)}
          >
            ...
            >
              <span className={`date ${isSunday ? "calendar_date_sunday" : ""}`}>
                {dayDate.getDate()}
              </span>
              <div className="calendar_app">
                <div className="appointment_body_1">
                  <div className="appointment_data_1">
                    <span className={`appointment_count_1 ${changedClassName}`}>
                      {day.status || "N/A"}
                    </span>
                  
                    <div  style={{ display: "flex", flexDirection: "column" }}>
                      <p>{day.time || "N/A"}</p>
                      <p>{day.patientname || "N/A"}</p>
                      <p>{day.priority || "N/A"}</p>
                      <p>{day.operationType || "N/A"}</p>
      ...
      <>
        <div className="calender_table_overall">
          ...
          <div className="RegisForm_1">
                ...
                >
                  <option value="">Select</option>
                  <option value="SENIORSURGEON">SENIOR SURGEON</option>
                  <option value="JUNIORSURGEON">JUNIOR SURGEON</option>
  ...
  //   calendarDays.forEach((day, index) => {
  //     if (day === null) {
  //       cells.push(<td key={`empty-${index}`} className="empty-cell"></td>);
  //     } else {
  //       cells.push(
  //         <td
  //           key={day.date}
  //           className={`calendar-cell ${day.status === "Booked" ? "booked" : "available"}`}
  //         >
  //           <div>{new Date(day.date).getDate()}</div>
  //           <div className="status">{day.status}</div>
  //           <div className="status">{day.btime}</div>
  //           <div className="status">{day.operationType}</div>
  //           <div className="status">{day.priority}</div>
  //         </td>
  //       );
  //     }
  
  //     // Add rows every 7 cells
  //     if ((index + 1) % 7 === 0 || index === calendarDays.length - 1) {
  //       rows.push(<tr key={`row-${index}`}>{cells.splice(0, cells.length)}</tr>);
  //     }
  //   });
