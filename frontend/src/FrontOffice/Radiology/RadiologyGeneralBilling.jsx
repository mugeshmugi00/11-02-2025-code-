...
function RadiologyGeneralBilling() {
  ...
  const fetchData = () => {
    const params = {
      Register_Id: OP_BillingData?.RegistrationId,
      RegisterType: OP_BillingData?.RegisterType
    };

    axios.get(`${UrlLink}OP/Radiology_Request_Detailslink_unpaid`, { params })
      .then((res) => {
        const ress = res.data;
        console.log("1234", ress);
        // setyes(ress?.IsSubTestYes);s
  ...
  useEffect(() => {
    const { RegistrationId, RegisterType, Status } = OP_BillingData || {};
  
    if (UrlLink && RegistrationId && RegisterType && Status === 'Unpaid') {
      fetchData();
    }
  }, [
    UrlLink,
    OP_BillingData?.RegistrationId,
    OP_BillingData?.RegisterType,
    OP_BillingData?.Status,  
  ]);
  ...
  // Track previous state of SelectDatalist and totals
  const prevSelectDatalistRef = useRef([]);
  ...
  useEffect(() => {
    const previousDatalist = prevSelectDatalistRef.current || [];
    const previousTotals = prevTotalsRef.current;

    // Find newly added items
    const newItems = SelectDatalist.slice(previousDatalist.length);
    ...
    newItems.forEach((item) => {
      newTotals.totalUnits += +item.Quantity || 0;
      newTotals.totalAmount += parseFloat(item.Charges) || 0;
      newTotals.totalDiscount += parseFloat(item.Discount) || 0;
      newTotals.totalGstamount += parseFloat(item.GST) || 0;
      newTotals.totalNetAmount += parseFloat(item.NetAmount) || 0;
    });

    // Update totals by adding new items' values to previous totals
    const updatedTotals = {
      totalNetAmount: previousTotals.totalNetAmount + newTotals.totalNetAmount,
      totalUnits: previousTotals.totalUnits + newTotals.totalUnits,
      totalDiscount: previousTotals.totalDiscount + newTotals.totalDiscount,
      totalGstamount: previousTotals.totalGstamount + newTotals.totalGstamount,
    };
    ...
    setinitialState((prevState) => ({
      ...
      totalDiscount: updatedTotals.totalDiscount.toFixed(2),
      totalGstamount: updatedTotals.totalGstamount.toFixed(2),
      totalNetAmount: updatedTotals.totalNetAmount.toFixed(2),
      totalAmount: updatedTotals.totalNetAmount.toFixed(2),
      BalanceAmount: (
        updatedTotals.totalNetAmount - prevState.PaidAmount
      ).toFixed(2), // Adjust BalanceAmount based on new totals
      Roundoff: (
        Math.round(updatedTotals.totalNetAmount) - updatedTotals.totalNetAmount
      ).toFixed(2),
    }));

    // Update references for next render
    prevSelectDatalistRef.current = [...SelectDatalist];
  ...
  const addbillingitem = (row) => {
    const newItem = {
      S_No: SelectDatalist.length + 1, // Auto-increment S.No
      ServiceType: row.RadiologyName,
      ServiceCode : row.TestCode,  // Map RadiologyName to ServiceType
      SelectItemName: row.TestName,    // Map TestName to SelectItemName
      Charges: row.Amount,             // Map Amount to Charges
      Quantity: 1,                     // Default quantity
      Amount: row.Amount,              // Map Amount
      DiscountType: "-",               // Default discount type
      Discount: "-",                   // Default discount
      NetAmount: row.Amount,           // Map Amount to NetAmount
      Status: "-",                     // Default status
    };
  
    // Add the new item to the SelectDatalist
    setSelectDatalist((prevList) => [...prevList, newItem]);
  
    // Remove the added test from the 'no' state
    setno((prevNo) => prevNo.filter(testName => testName !== row.TestName));
  };
      ...
      axios
        ...
        .then((res) => {
          ...
          if (Object.values(GetIPBillingdata).length > 5) {
            ...
            setBillingData((prev) => ({
              ...prev,
              DoctorName: OP_BillingData.Doctor_ShortName || "",
              DoctorId: OP_BillingData.DoctorId || "",
            }));
            setpatientsearch((prev) => ({
              ...prev,
              Search: OP_BillingData?.PatientId,
  ...
  const handleSubmit = (
    ...
    if (Object.keys(OP_BillingData).length !== 0) {
      axios.post(`${UrlLink}Frontoffice/RadiologyBilling_Link`, senddata)
        .then((res) => {
              ...
              <button onClick={handlesubmit}>Save</button>
                    ...
                    <div className="Service_bill">
                      {
                        Object.keys(ServiceProcedure || {})
                          .filter((p) => p !== 'ServiceSubCatId' && p !== 'Subcategorypk' && p !== 'Categoryid')
                          .map((field, indx) => (
                            <div className="RegisForm_1" key={indx}>
                              ...
                              <datalist id={`${field}_ServiceCategory`}>
                                {field === 'ServiceCategory' && (
                                  ServiceCategoryData?.map((row) => (
                                    <option key={row.service_category_id} value={`${row.service_category_id}-${row.service_name}`}>
                                      {row.service_name}
                                    </option>
                                  ))
                                )}
                                {field === 'ServiceSubCategory' && (
                                  ...
                                  ServiceSubCategoryData.map((row) => (
                                    <option key={row.pk} value={`${row.pk}-${row.service_subcategory}`}>
                ...
                { no?.length > 0 && (
                    ...
                    <th>Status</th> */}
                      ...
                      <td>-</td> 
                      <td>{row.Amount}</td> 
                      <td>-</td> Status is not provided in the data */}
                                  <td>
                                  <button
                                    className="delnamebtn"
                                    onClick={() => addbillingitem(row)} 
                                  >
                                    Add
                                  </button>
                                  </td>
                        ...
                        {/* Rest of your code for summary, payment method, etc. */}
                      </div>
                      </>
                )}
...
export default RadiologyGeneralBilling
