...
const LabTest = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${UrlLink}Masters/All_Other_Lab_Masters_POST_AND_GET?Type=Get_AllTests&gender=${DoctorWorkbenchNavigation.Gender}&Department=${formData?.SubDepartment_Code}`
      )
      .then((response) => {
  ...
  }, [UrlLink, DoctorWorkbenchNavigation.Gender, formData?.SubDepartment_Code]);
    ...
    const fetchPDF = async () => {
      try {
        const response = await axios.get(
          `${UrlLink}OP/ReportData?Request_Id=${DoctorWorkbenchNavigation?.LabRequestId}`,
          {
