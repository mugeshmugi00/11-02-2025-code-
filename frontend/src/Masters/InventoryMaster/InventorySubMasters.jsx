...
const InventorySubMasters = () => {
    ...
    useEffect(() => {
        ...
        }else if (StrengthState.ProductCategoryName !== "") {
            productCategoryName = StrengthState.ProductCategoryName
        }
        ...
        StrengthState.ProductCategoryName,
        UrlLink,
    ]);
    // -----------------------------------------------------------------------
...
const StrengthNamedata = useCallback(() => {

    axios.get(`${UrlLink}Inventory/StrengthName_Master_Link`)
        .then((res) => {
            console.log(res.data);
            let data = res.data

            if (Array.isArray(data) && data.length !== 0) {
                setStrengtharray(data)
            }
            else {
                setStrengtharray([])
            }
...
const HandeleEditStrengthName = (params) => {
    let Editdata = params

    setStrengthState({
        StrengthId: Editdata.id,
        StrengthName: Editdata.StrengthName,
        ProductCategoryName: Editdata.ProductCategory,
        SubProductCategoryName: Editdata.SubProductCategory
    });
...
const HandleUpdateStatusStrengthName = (params) => {

    let Editdata = {
        StrengthId: params.id,
        Statusedit: true
    }

    axios.post(`${UrlLink}Inventory/StrengthName_Master_Link`, Editdata)
        .then((res) => {
            // console.log(res.data);
            let data = res.data
...
const StrengthNameColumn = [
    ...
    },
    {
        ...
        renderCell: (params) => (
            <>
                ...
                <Button
                    className="cell_btn"
                    onClick={() => HandleUpdateStatusStrengthName(params.row)}
                >
                    {params.row.Status ? "ACTIVE" : "INACTIVE"}
                </Button>
    ...
    },
    {
        ...
        renderCell: (params) => (
            <>
                ...
                <Button
                    className="cell_btn"
                    onClick={() => HandeleEditStrengthName(params.row)}
                >
                    <EditIcon className="check_box_clrr_cancell" />
                </Button>
  ...
  const handleChangeStrengthName = useCallback((e) => {
        const { name, value } = e.target;
    ...
    const HandleSaveStrengthState =()=>{

        let requiredFields =['ProductCategoryName','SubProductCategoryName','StrengthName']
    
        const missingFields = requiredFields.filter(
            (field) => !StrengthState[field]
        )
        ...
        if (missingFields.length !== 0) {
            ...
            console.log('StrengthState',StrengthState);
            ...
            axios.post(`${UrlLink}Inventory/StrengthName_Master_Link`, senddata)
            .then((res) => {
                // console.log(res.data);
            ...
            <div className="Main_container_app">
                ...
                <div className="RegisterTypecon">
                    <div className="RegisterType">
                        ...
                        {["ProductType", "ProductGroup", "PackType", "GenericName", "ManufacturerName",'Strength'].map((p, ind) => (
                ...
                {RackMasterPage === 'Strength' && <>
                    ...
                    <div className="RegisForm_1">
                            ...
                            <select
                                name="ProductCategoryName"
                                value={StrengthState.ProductCategoryName}
                                onChange={handleChangeStrengthName} 
                            >
                                <option value="">Select</option>
                                {Array.isArray(ProductCategoryData) &&
                                    ProductCategoryData
                        ...
                        <div className="RegisForm_1">
                            ...
                            <select
                                name="SubProductCategoryName"
                                value={StrengthState.SubProductCategoryName}
                                onChange={handleChangeStrengthName} // Corrected from onchange to onChange
                            >
                                <option value="">Select</option>
                                {Array.isArray(SubCategoryData) &&
                                    SubCategoryData
                        ...
                        <div className="RegisForm_1">
                            <label>Strength Name<span>:</span> </label>
                            <input
                                type="text"
                                name='StrengthName'
                                autoComplete='off'
                                required
                                value={StrengthState.StrengthName}
                                onChange={(e) => setStrengthState((prev) => ({ ...prev, StrengthName: e.target.value }))}
                            />
                        </div>
                    ...
                    <div className="Main_container_Btn">
                        <button
                            onClick={HandleSaveStrengthState}
                        >
                            {StrengthState.StrengthId ? "Update" : "Add"}
                        </button>
                    </div>

                    <div className='RegisFormcon_1 jjxjx_'>
                    <ReactGrid columns={StrengthNameColumn} RowData={Strengtharray} />
                    </div>
