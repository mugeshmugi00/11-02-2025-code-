...
const ProductMasterList = () => {
    ...
    useEffect(() => {
        const fetchdata = async () => {
            try {
                ...
                setCategoryArray(data1?.data && Array.isArray(data1?.data) && data1?.data.length > 0 ? data1?.data : [])

                setManufactureData(data3?.data && Array.isArray(data3?.data) && data3?.data.length > 0 ? data3?.data : [])
            } catch (error) {
                console.error(error);
    ...
    const handelEditdata = useCallback((params) => {
        ...
        if (params?.Status === 'Active') {
            ...
            dispatchvalue({ type: "HrFolder", value: "Productmaster" });
