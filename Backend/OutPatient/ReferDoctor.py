...
def Radiology_Request_Detailslink(request):
    if request.method == 'POST':
        try:
            ...
            if RegisterType == "OP" and RegisterType == 'RADIOLOGY':
        ...
        try:
            ...
            print(registration_id,'registration_id',RegisterType,'RegisterType')
            ...
            print(registration_details,'registration_details',radiology_requests,'radiology_requests')
            ...
            for req in radiology_requests:
                    ...
                    print(testname,'testname',radiologyname,'radiologyname')
...
def Radiology_Request_Detailslink_unpaid(request):
    try:
            def get_file_image(filedata):
                if filedata is None:
                    return None  # Ret
            ...
            if RegisterType == "OP":
                radiology_requests = Radiology_Request_Details.objects.filter(OP_Register_Id=registration_id, RegisterType=RegisterType, Status='Unpaid')
            elif RegisterType == "IP":
                radiology_requests = Radiology_Request_Details.objects.filter(IP_Register_Id=registration_id, RegisterType=RegisterType, Status='Unpaid')
            elif RegisterType == "Casuality":
                radiology_requests = Radiology_Request_Details.objects.filter(Casuality_Register_Id=registration_id, RegisterType=RegisterType, Status='Unpaid')
            elif RegisterType == "ExternalRadiology":
                radiology_requests = Radiology_Request_Details.objects.filter(Diagnosis_Register_Id=registration_id, RegisterType=RegisterType, Status='Unpaid')
...
@csrf_exempt
@require_http_methods(["POST","GET","OPTIONS"])
def Radiology_TestAmount_Entry(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for testname in Testname_arr:
                Radiologyid = testname.get('Radiologyid', None)
                TestCode = testname.get('TestCode', None)
                TestAmount = testname.get('TestAmount',None)
                Status = testname.get('Status',None)
                ...
                if Status == 'Paid':
                    ...
                    if RegisterType == 'OP' or RegisterType == "RADIOLOGY":
                        ...
                        radiology_ins = Radiology_Request_Details.objects.get(TestCode = Radiologyid,SubTestCode=TestCode,OP_Register_Id_id = registration_id) 
                        radiology_ins.Status = 'Paid'
                        radiology_ins.save()

                if Radiologyid and TestCode :


                    test_ins = TestName_Details.objects.get(Test_Code=TestCode,Radiology_Id_id=Radiologyid)

                    if TestAmount:

                        test_ins.Amount = TestAmount
                        test_ins.save()
                    ...
                    if RegisterType == 'OP' or RegisterType == "RADIOLOGY":
                        ...
                        radiology_ins = Radiology_Request_Details.objects.get(TestCode = Radiologyid,SubTestCode=TestCode,OP_Register_Id_id = registration_id) 
                        radiology_ins.Status = 'Unpaid'
                        radiology_ins.save()

            return JsonResponse({'success': 'updated successfully'})

                
                # if Radiologyid and TestCode and registration_id :  # Ensure required fields are present
                #     Radiology_Request_Details.objects.create(
                #         RegisterType = RegisterType,
                #         OP_Register_Id=registration_instance if RegisterType == "OP" else None,
                #         IP_Register_Id=registration_instance if RegisterType == "IP" else None,
