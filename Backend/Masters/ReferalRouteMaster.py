            ...
            if RouteId:
                if Statusedit:
                    ...
                    if Route_Master_Detials.objects.filter(
                        Q(Route_No=RouteNo) & Q(location=location_instance)
                    ).exists():
                        if Route_Master_Detials.objects.filter(
                            Q(Route_No=RouteNo) & Q(location=location_instance) & Q(Route_Name=RouteName) &
                            Q(Teshil_Name=TahsilName) & Q(Village_Name=VillageName)
                        ).exists():
                            return JsonResponse({'warn': f"A RouteNo - {RouteNo} with the same RouteName, Tahsil, and Village Name already exists in the same Location."})
